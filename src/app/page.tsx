      ...
      <section id="pricing" className="mx-auto max-w-7xl px-6 py-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`rounded-2xl p-6 flex flex-col ${plan.highlighted
                  ? "bg-orange-500 text-white shadow-xl shadow-orange-200"
                  : "bg-white border border-gray-100"
                }`}
            >
              <div
                className={`text-sm font-semibold uppercase tracking-wider mb-2 ${plan.highlighted ? "text-orange-100" : "text-gray-400"
                  }`}
              >
                {plan.name}
              </div>
              <div className="flex items-end gap-1 mb-1">
                ...
                <span
                  className={`text-sm mb-1 ${plan.highlighted ? "text-orange-100" : "text-gray-400"
                    }`}
                >
                  {plan.per}
                </span>
              ...
              <ul className="flex-1 space-y-2 mb-8">
                {plan.features.map((feat) => (
                  <li key={feat} className="flex items-start gap-2 text-sm">
                    <span
                      className={`mt-0.5 ${plan.highlighted ? "text-orange-200" : "text-orange-500"
                        }`}
                    >
                      ✓
                    </span>
              ...
              <Link
                href={plan.href}
                className={`rounded-full py-2.5 text-center text-sm font-semibold transition-colors ${plan.highlighted
